...
## Evolution and cooperation rate charts for sensitivity analysis, large-population scenario.
## Simulated data for the money scenario, with variations in liquidity and benefit-cost parameters.
## Chart included in the supplementary information document.
...
df_large_population <- 
  here("Simulation data", "Supplementary data - large population.csv") %>% 
  read.csv(skip = 6) %>% 
  clean_names() %>%
  as_tibble()  %>% 
...
# 3. Draw main evolution plot ---------------------------------------------

# Plot cooperation rates and share of surviving strategies in time (with money)
p_large_population = df_large_population %>% 
  ggplot(aes(x=step)) +
  ylim(0, 1) +
  stat_summary(
  ...
  labs(
    ...
    #title = "With money: evolution of surviving strategies and cooperation rates (Median and IQR over 100 repetitions; total population = 1500)"
  ) +
