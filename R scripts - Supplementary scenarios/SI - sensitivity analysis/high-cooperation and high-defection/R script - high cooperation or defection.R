...
## Evolution and cooperation rate charts for sensitivity analysis, high-cooperation and high-defection scenarios.
...
# 2.1 Simulation data with high initial proportion of defectors
df_high_defection <- 
  here("Simulation data", "Supplementary data - high-defection.csv") %>% 
  read.csv(skip = 6) %>% 
  clean_names() %>%
  as_tibble()  %>% 
...
# 2.1 Simulation data with high initial proportion of cooperators
df_high_cooperation <- 
  here("Simulation data", "Supplementary data - high-cooperation.csv") %>% 
  read.csv(skip = 6) %>% 
  clean_names() %>%
  as_tibble()  %>% 
  select(-starts_with("sum_fit"), -starts_with("sum_bal"),-starts_with("sum_sco"), -starts_with(("x_sum"))) %>%
...
# 3.1 Plot cooperation rates and share of surviving strategies in time (high-defection)
p_high_defection = df_high_defection %>% 
  ggplot(aes(x=step)) +
  ylim(0, 1) +
  stat_summary(
...
# 3.1 Plot cooperation rates and share of surviving strategies in time (high-cooperation)
p_high_cooperation = df_high_cooperation %>% 
  ggplot(aes(x=step)) +
  ylim(0, 1) +
  stat_summary(
    aes(y = cooperation_rate, shape = ""),
